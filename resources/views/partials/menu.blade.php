    ...
    <div class="collapse navbar-collapse  w-auto  max-height-vh-100 h-100" id="sidenav-collapse-main">
      <ul class="navbar-nav">
        ...
        @can('healthcare_access')
          <li class="nav-item mt-3">
            <h6 class="ps-4 ms-2 text-uppercase text-xs font-weight-bolder opacity-6">{{ trans('cruds.healthcare.title') }}</h6>
          </li>
          @can('blood_pressure_access')
            <li class="nav-item">
              <a href="{{ route("admin.healthcare.blood-pressures.index") }}" class="nav-link {{ request()->is("admin/healthcare/blood-pressures") || request()->is("admin/healthcare/blood-pressures/*") ? "active" : "" }}">
                <div class="icon icon-shape icon-sm shadow border-radius-md bg-primary text-center me-2 d-flex align-items-center justify-content-center">
                  <i class="fa-fw fas fa-heartbeat c-sidebar-nav-icon"></i>
                </div>
                <span class="nav-link-text ms-1">{{ trans('cruds.blood_pressure.title') }}</span>
              </a>
            </li>
          @endcan
        @endcan
